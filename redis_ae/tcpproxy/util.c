...
{
    log_level=level;
    if(filename==NULL||strcmp(filename,"stderr")==0 \
	    || strcmp(filename,"")==0) {
	log_file=stderr;
...
BufferList*AllocBufferList(int n)
{
    ...
    buf->next=NULL;
    blist->head=buf;
    blist->head=blist->write_node=buf;
    pre=buf;
...
{
    if(blist->head==blist->write_node&&
	    blist->read_pos==blist->head->size){
	*len=0;
	LogDebug("head empyt");
	return NULL;
    }
...
//pop out already sending data
void BufferListPop(BufferList*blist,int len)
{
    blist->read_pos+=len;
    LogDebug("popout");
    if(blist->read_pos==blist->head->size&&
	    blist->head!=blist->write_node){
	BufferListNode*cur=blist->head;

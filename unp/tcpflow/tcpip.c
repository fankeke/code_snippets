...
void process_ip(const u_char *data, u_int32_t caplen)
{
  ...
  switch(ip_header->ip_p){
      case IPPROTO_ICMP:
	  update_pkt_type(PROTO_ICMP_,caplen+14);
	  break;
      case IPPROTO_UDP:
	  update_pkt_type(PROTO_UDP_,caplen+14);
	  break;
      case IPPROTO_TCP:
	  update_pkt_type(PROTO_TCP_,caplen+14);
	  tcp_conver_handler(data,ip_header_len,caplen+14);//我只能放在这
	  						  //里了，放在
							  //后面的话ip头部
							  //很麻烦计算
	  break;
  ...
  if (ip_header->ip_p == IPPROTO_TCP) {
  	...
  	process_udp(data + ip_header_len, ip_total_len - ip_header_len,
	      ntohl(ip_header->ip_src.s_addr),
	      ntohl(ip_header->ip_dst.s_addr));
  }
  update_summary_file(); /*在此处更新summary文件*/
  update_tcp_conver_file();

}
void process_udp(const u_char *data, u_int32_t length, u_int32_t src,
		 u_int32_t dst)
{
 ...
 // tcp_header_len = tcp_header->th_off * 4;
  ...
  /*this_flow.src = src;
  this_flow.dst = dst;*/
  this_flow.sport = ntohs(udp_header->uh_sport);
  this_flow.dport = ntohs(udp_header->uh_dport);

  if(this_flow.sport==53||this_flow.dport==53)
      update_pkt_type(PROTO_DNS_,length+20+14);
		 ...
		 u_int32_t dst)
{
  ...
  if (length <= tcp_header_len) {
    ...
    if(this_flow.sport==80||this_flow.dport==80)
      update_pkt_type(PROTO_HTTP_ack,length+20+14);
    else if(this_flow.sport==443||this_flow.dport==443)
      update_pkt_type(PROTO_SSL_ack,length+20+14);
  ...
  if(this_flow.sport==80||this_flow.dport==80)
      update_pkt_type(PROTO_HTTP_,length+20+14);
  else if(this_flow.sport==443||this_flow.dport==443)
      update_pkt_type(PROTO_SSL_,length+20+14);
...
{
  static u_char buf[SNAPLEN]; u_char *write_ptr;

...
void dl_ethernet(u_char *user, const struct pcap_pkthdr *h, const u_char *p)
{
  ...
  if (length != caplen) {
    DEBUG(6) ("warning: only captured %d bytes of %d byte ether frame", caplen, length);
  }
  ...
  if(ntohs(eth_header->ether_type)==ETHERTYPE_ARP)
      update_pkt_type(PROTO_ARP_,length);

  detect_encrypted_stream(p,caplen);
  print_packet_orig(user,h,p);
  ...
  update_pkt_type(PROTO_IP_,length);
